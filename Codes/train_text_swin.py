...
# --- Project Packages ---
from utils import save_checkpoint, load_checkpoint, train_one_epoch, evaluate
...
if DATASET_NAME == 'MIMIC':
    ...
    BATCH_SIZE = 16 if PHASE == 'TRAIN' else 16 # Fit 1 GPU
...
if __name__ == "__main__":
    ...
    checkpoint_path_to = 'checkpoints_swin/{}_{}_{}.pt'.format(DATASET_NAME,MODEL_NAME,COMMENT)
    
    if RELOAD:
        last_epoch, (best_metric, test_metric) = load_checkpoint(checkpoint_path_from, model, optimizer, scheduler)
    ...
    if PHASE == 'TRAIN':
        ...
        for epoch in range(last_epoch+1, EPOCHS):
            print('Epoch:', epoch)
            train_loss = train_one_epoch(train_loader, model, optimizer, criterion, device='cuda', kw_src=KW_SRC, kw_tgt=KW_TGT, kw_out=KW_OUT, scaler=scaler)
            ...
            if best_metric < val_metric:
                ...
                save_checkpoint(checkpoint_path_to, model, optimizer, scheduler, epoch, (val_metric, test_metric))
                print('New Best Metric: {}'.format(best_metric))
                print('Saved To:', checkpoint_path_to)
            
    elif PHASE == 'TEST':
        test_loss, test_outputs, test_targets = evaluate(test_loader, model, criterion, device='cuda', kw_src=KW_SRC, kw_tgt=KW_TGT, kw_out=KW_OUT)
