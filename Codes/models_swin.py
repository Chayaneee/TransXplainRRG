...
class ViT(nn.Module):
    ...
    def __init__(self, model, model_type='deit'):
        super().__init__()
        self.model_type = model_type
        self.model = model
        #self.model = nn.DataParallel(model)
        
        if 'deit' in model_type.lower():
            modules = list(self.model.children())[:-1]
            self.feature = nn.Sequential(*modules[:-1])
            self.average = model.norm #model.norm
    ...
    def forward(self, input):
        if self.model_type == 'Swin':
           outputs = self.feature(input)
            # Assuming the key for the last hidden state is 'last_hidden_state'
           outputs = outputs['last_hidden_state']
           outputs = outputs.transpose(1, 2)  # Transpose to match the expected dimensions for pooling
        else:
            outputs = self.feature(input)
        
         
        #outputs = self.feature(input)
        image_embed = self.average(outputs)
        
        return image_embed  # (B,L,E); (B,E,L) for Swin Transformer so need to change in MViT
...
class MViT(nn.Module):
    ...
    def forward(self, input):
        ...
        img_embed = self.model(img) # (B*V, L, E)
        img_embed = img_embed.view(B,V,img_embed.shape[-2], img_embed.shape[-1]) # (B,V,L,E)
        #print(img_embed.shape)

        msk = (pos == -1) # (B,V)
        msk_embed = msk.view(B,V,1,1).float() # (B,V,1,1) * (B,V,L,E)
        img_embed = msk_embed * (-1) + (1-msk_embed) * img_embed
        
        #print(img_embed.shape)

        image_embed = img_embed.max(dim=1)[0] # (B,L,E)
        
        #print("Shape as B,E,L",image_embed.shape)
        
        image_feature = image_embed.max(dim=2)[0]  ### for Swin-T model
        
        #image_feature = image_embed.max(dim=1)[0] # for DeiT-384 model
        
        #print("Shape as B,E",image_feature.shape)

        return image_feature  # (B,E)
...
class CNN(nn.Module):
     def __init__(self, model, model_type='resnet'):
        ...
        # # DeiT model features and identity module is taken    
        # elif 'deit' in model_type.lower(): # deit-224, deit-384
     ...
     def forward(self, input):
        ...
        #print(wxh_features.shape)     
        avg_features = self.average(wxh_features) # (B,2048,1,1)
        
        if 'deit' in self.model_type.lower():
            avg_features = avg_features  #(B,768)
        else:
...
class MVCNN(nn.Module):
    ...
    def forward(self, input):
        ...
        # Modification for DeiT
        
        avg = avg.view(B,V,avg.shape[-2], avg.shape[-1]) # (B,V,F)
        
        #print('Average_feture is : {}'.format(avg.shape))
        #wxh = wxh.view(B,V,wxh.shape[-3],wxh.shape[-2],wxh.shape[-1]) # (B,V,F,W,H)
        
        #
        
        wxh = wxh.view(B,V,wxh.shape[-2],wxh.shape[-1]) # (B,V,F,W,H)
        
        #print('WxH_feture is : {}'.format(wxh.shape))
        
        
        msk = (pos == -1) # (B,V)
        msk_wxh = msk.view(B,V,1,1).float() # (B,V,1,1,1) * (B,V,F,C,W,H)
        msk_avg = msk.view(B,V,1,1).float() # (B,V,1) * (B,V,F)
        ...
        #avg_features = avg_features.max(dim=1)[0] # (B,F)
        
        #print(wxh_features.shape)
        #print("Final feature is {}".format(avg_features.shape))
        return avg_features, wxh_features


# --- Main Moduldes ---
class Classifier(nn.Module):
    def __init__(self, num_topics, num_states, cnn=None, vit=None, tnn=None,
                fc_features=768, embed_dim=128, num_heads=1, dropout=0.5):
        super().__init__()
        ...
        self.vit_features = nn.Linear(fc_features, num_topics * embed_dim) if vit != None else None
    ...
    def forward(self, img=None, txt=None, lbl=None, txt_embed=None, pad_mask=None, pad_id=3, threshold=0.5, get_embed=False, get_txt_att=False):
        ...
        if img != None: # (B,C,W,H) or ((B,V,C,W,H), (B,V))
            ...
            vit_features = self.vit(img) # (B,F)
            #print("Image feature is {}" .format(vit_features.shape))
        ...
        if img != None and (txt != None or txt_embed != None):
            topic_index = torch.arange(self.num_topics).unsqueeze(0).repeat(vit_features.shape[0],1).to(vit_features.device) # (B,T)
            state_index = torch.arange(self.num_states).unsqueeze(0).repeat(vit_features.shape[0],1).to(vit_features.device) # (B,C)
            ...
            vit_features = self.vit_features(vit_features).view(vit_features.shape[0], self.num_topics, -1)
            txt_features, txt_attention = self.txt_features(txt_features, topic_embed, pad_mask) # (B,T,E), (B,T,L)
            final_embed = self.normalize(vit_features + txt_features) # (B,T,E)
            
        elif img != None:
            topic_index = torch.arange(self.num_topics).unsqueeze(0).repeat(vit_features.shape[0],1).to(img_features.device) # (B,T)
            state_index = torch.arange(self.num_states).unsqueeze(0).repeat(vit_features.shape[0],1).to(img_features.device) # (B,C)
            topic_embed = self.topic_embedding(topic_index) # (B,T,E)
            state_embed = self.state_embedding(state_index) # (B,C,E)

            # img_features = self.img_features(img_features).view(img_features.shape[0], self.num_topics, -1) # (B,F) --> (B,T*E) --> (B,T,E)   
            
            vit_features = self.vit_features(vit_features).view(vit_features.shape[0], self.num_topics, -1)  # (B,T,E)
            final_embed = vit_features # (B,T,E)
