...
class MIMIC(data.Dataset): # MIMIC-CXR Dataset
    def __init__(self, directory, input_size=(384,384), random_transform=True,
                view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image','history'], targets=['label'], 
                max_len=1000, vocab_file='Vocabulary/mimic_unigram_5000.model'):

        self.source_sections = ['INDICATION:', 'HISTORY:', 'CLINICAL HISTORY:', 'REASON FOR EXAM:', 'INTERPRETATION:', 'REASON FOR EXAMINATION:', 'CLINICAL INFORMATION:', 'CLINICAL INDICATION:', 'PATIENT HISTORY:']
    ...
    def __getitem__(self, idx):
        ...
        if 'image' in self.sources:
            ...
            for i in range(min(self.max_views,len(img_files))):
                img_file = self.dir + 'files/' + idx[0] + '/' + idx[1] + '/' + img_files[i]
                #print(img_file)
    ...
    def __get_reports_images(self, file_name='reports.json'):    #### reports.json is available in MIMIC dataset downloaded from Physionet.org
        ...
        for file_name, report in caption_file.items():
           ...
           # print(a)
            ...
            try:
                ...
                file_list = os.listdir(self.dir + 'files/' + pid + '/' + sid ) 
               # print(file_list[0])
    ...
    def __get_labels(self, binary_mode, file_name='/mnt/data/chayan/Mini-MIMIC/Filtered-mimic-cxr-2.0.0-chexpert-train.csv'):
    ...
    def __get_nounphrase(self, top_k=100, file_name='/home/chayan/TransXplainNet+/Codes/count_nounphrase.json'):
        count_np = json.load(open(file_name, 'r'))
    ...
    def __generate_splits(self, file_name="/mnt/data/chayan/Mini-MIMIC/Filtered-mimic-cxr-2.0.0-chexpert-train.csv"):  
        train_val_file = open('/mnt/data/chayan/Mini-MIMIC/train_val_list.txt', 'w')  
        print("Create val_list data" )
        test_file = open('/mnt/data/chayan/Mini-MIMIC/mimic_test_list.txt', 'w')  
        print("Create test_list data" )

        train_file = "/mnt/data/chayan/Mini-MIMIC/Filtered-mimic-cxr-2.0.0-chexpert-train.csv" 
        train_data = pd.read_csv(train_file, dtype=object)
        train_data = train_data.to_numpy().astype(str)
        
        train_pid_sid = {}
        for i in range(len(train_data)):
            pid = train_data[i, 0].item()
            sid = train_data[i, 1].item()
            
            if pid in train_pid_sid:
                train_pid_sid[pid].append(sid)
            else:
                train_pid_sid[pid] = [sid]

        unique_train_pid = np.unique(train_data[:, 0])

        for pid in unique_train_pid:
            for sid in train_pid_sid[pid]:
                if ('p' + pid, 's' + sid) in self.img_captions:
                    train_val_file.write('p' + pid + '/' + 's' + sid + '\n')
        
        test_dir = "/mnt/data/chayan/Mini-MIMIC/Filtered-mimic-cxr-2.0.0-chexpert-test.csv" 
        test_data = pd.read_csv(test_dir, dtype=object)
        test_data = test_data.to_numpy().astype(str)

        # 1 PID can have multiple SIDs
        test_pid_sid = {}
        for i in range(len(test_data)):
            pid = test_data[i, 0].item()
            sid = test_data[i, 1].item()
            
            if pid in test_pid_sid:
                test_pid_sid[pid].append(sid)
            else:
                test_pid_sid[pid] = [sid]

        unique_test_pid = np.unique(test_data[:, 0])

        for pid in unique_test_pid:
            for sid in test_pid_sid[pid]:
                if ('p' + pid, 's' + sid) in self.img_captions:
    ...
    def get_subsets(self, pvt=0.9, seed=0, generate_splits=True, debug_mode=False, train_phase=True):
        ...
        train_files = np.loadtxt('/mnt/data/chayan/Mini-MIMIC/train_val_list.txt', dtype=str)
        test_files = np.loadtxt('/mnt/data/chayan/Mini-MIMIC/mimic_test_list.txt', dtype=str)
        ...
        print("No of train indices after split:",len(train_indices))
        
        val_indices = indices[pivot:]
        
        print("No of validation indices after split:",len(val_indices))
        ...
        print('Validation dataset size is {}'.format(len(val_dataset)))
        ...
        print('Train dataset size is {}'.format(len(train_dataset)))
        print('Validation dataset size is {}'.format(len(val_dataset)))
...
class NLMCXR(data.Dataset): # Open-I Dataset
    def __init__(self, directory, input_size=(256,256), random_transform=True,
                view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image'], targets=['label'], ## if history is included then sources=['image','history']
                max_len=1000, vocab_file='Vocabulary/nlmcxr_unigram_1000.model'):
        
        self.source_sections = []  #if history is included then  ['INDICATION', 'COMPARISON']
        self.target_sections = ['FINDINGS']
        self.vocab = spm.SentencePieceProcessor(model_file=directory + vocab_file)
        self.vocab_file = vocab_file # Save it for subsets
    ...
    def __getitem__(self, idx):
        ...
        if 'image' in self.sources:
            ...
            for i in range(min(self.max_views,len(img_files))):
                img_file = self.dir + 'images/images/' + img_files[i] + '.png'
                img = Image.open(img_file).convert('RGB')
                imgs.append(self.transform(img).unsqueeze(0)) # (1,C,W,H)
...
class NLMCXR_Test(data.Dataset): # Open-I Dataset
    def __init__(self, directory, input_size=(384,384), random_transform=True,
                view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image'], targets=['label'], ## if history is included then sources=['image','history']
                max_len=1000, vocab_file='Vocabulary/mimic_unigram_5000.model'):
        
        self.source_sections = ['INDICATION', 'COMPARISON']  #if history is included then  ['INDICATION', 'COMPARISON']
        self.target_sections = ['FINDINGS']
        self.vocab = spm.SentencePieceProcessor(model_file=directory + vocab_file)
        self.vocab_file = vocab_file # Save it for subsets
    ...
    def get_subsets(self, train_size=0, val_size=0, test_size=1, seed=0):
        np.random.seed(seed)
        indices = np.random.permutation(len(self.file_list))
        train_pvt = int(train_size * len(self.file_list))
        val_pvt = int((train_size + val_size) * len(self.file_list))
